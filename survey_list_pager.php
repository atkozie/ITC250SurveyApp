...
<p>This page is the entry point of the application, meaning this page gets a link on your web site.  Since the current subject is surveys, we could name the link something clever like <a href="<?php echo VIRTUAL_PATH; ?>survey_list_pager.php">Surveys</a></p>
...
{#records exist - process
    ...
    echo '<div align="center"><h2>We have ' . $myPager->showTotal() . ' ' . $itemz . '!</h2></div>';
	while($row = mysqli_fetch_assoc($result))
	{# process each row
        echo '<table border="1" class="table">';
        echo '<tr><td>Title: <a href="' . VIRTUAL_PATH . 'surveys/survey_view.php?id=' . (int)$row['SurveyID'] . '">' . dbOut($row['Title']) . '</a></td></tr>';
        echo '<tr><td>Created by: ' . dbOut($row['AdminName']) . '</td></tr>';
        echo '<tr><td>Created on: ' . dbOut($row['DateAdded']) . '</td></tr>';
        echo '</table>';
	}
